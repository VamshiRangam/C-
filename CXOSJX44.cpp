//## begin module%1.10%.codegen_version preserve=yes
//   Read the documentation to learn more about C++ code generator
//   versioning.
//## end module%1.10%.codegen_version

//## begin module%67BEEF610034.cm preserve=no
//## end module%67BEEF610034.cm

//## begin module%67BEEF610034.cp preserve=no
//	Copyright (c) 1997 - 2022
//	FIS
//## end module%67BEEF610034.cp

//## Module: CXOSJX39%67BEEF610034; Package body
//## Subsystem: JXDLL%645AEC9A0298
//## Source file: C:\Repos\datanavigatorserver\Windows\Build\Dn\Server\Library\Jxdll\CXOSJX39.cpp

//## begin module%67BEEF610034.additionalIncludes preserve=no
//## end module%67BEEF610034.additionalIncludes

//## begin module%67BEEF610034.includes preserve=yes
//## end module%67BEEF610034.includes

#ifndef CXOSMN05_h
#include "CXODMN05.hpp"
#endif
#ifndef CXOSRU12_h
#include "CXODRU12.hpp"
#endif
#ifndef CXOSBS26_h
#include "CXODBS26.hpp"
#endif
#ifndef CXOSDB02_h
#include "CXODDB02.hpp"
#endif
#ifndef CXOSIF16_h
#include "CXODIF16.hpp"
#endif
#ifndef CXOSNS10_h
#include "CXODNS10.hpp"
#endif
#ifndef CXOSBS12_h
#include "CXODBS12.hpp"
#endif
//#ifndef CXOSES62_h
//#include "CXODES62.hpp"
//#endif
#ifndef CXOSJX44_h
#include "CXODJX44.hpp"
#endif


//## begin module%67BEEF610034.declarations preserve=no
//## end module%67BEEF610034.declarations

//## begin module%67BEEF610034.additionalDeclarations preserve=yes
#include "CXODES01.hpp"
#include "CXODRL04.hpp"
//## end module%67BEEF610034.additionalDeclarations


//## Modelname: DataNavigator Foundation::RESTCommand_CAT%645AEC55037E
namespace restcommand {
//## begin restcommand%645AEC55037E.initialDeclarations preserve=yes
//## end restcommand%645AEC55037E.initialDeclarations

// Class restcommand::RelatedCaseCommand 

 RelatedCaseCommand::RelatedCaseCommand()
  //## begin RelatedCaseCommand::RelatedCaseCommand%67BEF0690257_const.hasinit preserve=no
  //## end RelatedCaseCommand::RelatedCaseCommand%67BEF0690257_const.hasinit
  //## begin RelatedCaseCommand::RelatedCaseCommand%67BEF0690257_const.initialization preserve=yes
   : RESTCommand("/rest/datanavigator/resolve/relatedcases/v1.0.0", "S0003D", "@##JECASE ")
  //## end RelatedCaseCommand::RelatedCaseCommand%67BEF0690257_const.initialization
{
  //## begin restcommand::RelatedCaseCommand::RelatedCaseCommand%67BEF0690257_const.body preserve=yes
   memcpy(m_sID, "JX44", 4);
  //## end restcommand::RelatedCaseCommand::RelatedCaseCommand%67BEF0690257_const.body
}

 RelatedCaseCommand::RelatedCaseCommand (Handler* pSuccessor)
  //## begin RelatedCaseCommand::RelatedCaseCommand::RelatedCaseCommand%67BEF23001AA.hasinit preserve=no
  //## end RelatedCaseCommand::RelatedCaseCommand::RelatedCaseCommand%67BEF23001AA.hasinit
  //## begin RelatedCaseCommand::RelatedCaseCommand::RelatedCaseCommand%67BEF23001AA.initialization preserve=yes
   : RESTCommand("/rest/datanavigator/resolve/relatedcases/v1.0.0", "S0003D", "@##JECASE ")
  //## end restcommand::RelatedCaseCommand::RelatedCaseCommand%67BEF23001AA.initialization
{
  //## begin restcommand::RelatedCaseCommand::RelatedCaseCommand%67BEF23001AA.body preserve=yes
   memcpy(m_sID, "JX44", 4);
   m_pSuccessor = pSuccessor;
   m_hXMLText.add('R', &m_hGenericSegment);
   m_hXMLText.add('X', segment::SOAPSegment::instance());
   m_pXMLItem = new XMLItem();
   m_hQuery.attach(this);
  //## end restcommand::RelatedCaseCommand::RelatedCaseCommand%67BEF23001AA.body
}


 RelatedCaseCommand::~RelatedCaseCommand()
{
  //## begin restcommand::RelatedCaseCommand::~RelatedCaseCommand%67BEF0690257_dest.body preserve=yes
  //## end restcommand::RelatedCaseCommand::~RelatedCaseCommand%67BEF0690257_dest.body
}



//## Other Operations (implementation)
bool RelatedCaseCommand::execute ()
{
  //## begin restcommand::RelatedCaseCommand::execute%67BEF260020A.body preserve=yes
   UseCase hUseCase("CLIENT", "## JX44 READ RELATED CASES");
   if (!m_pXMLDocument)
#ifdef MVS
      m_pXMLDocument = new XMLDocument("JCL", "RJECASE", &m_hRow, &m_hXMLText);
#else
      m_pXMLDocument = new XMLDocument("SOURCE", "CXORJX44", &m_hRow, &m_hXMLText);
#endif
   m_pXMLDocument->reset();
   m_pXMLItem->reset(); 
   m_pXMLDocument->setMaximumSize(1000000);
   m_pXMLDocument->setSuppressEmptyTags(false);
   m_hQuery.reset();
   hQuery.reset();
   int i = parse();
   if (i != 0)
   {
      SOAPSegment::instance()->setRtnCde('3');
      m_pXMLDocument->add("details");
      return reply();
   }
   m_pXMLDocument->add("root");
   auto_ptr<SelectStatement> pSelectStatement((SelectStatement*)DatabaseFactory::instance()->create("SelectStatement"));
   if (!m_pXMLItem->get("caseNumber").empty() || !m_pXMLItem->get("primaryKey").empty())
   {
      if (!m_pXMLItem->get("caseNumber").empty())
      {
         m_iQuery = 1;
         hQuery.bind("EMS_CASE", "TSTAMP_TRANS", Column::STRING, &m_strTSTAMP_TRANS);
         hQuery.bind("EMS_CASE", "UNIQUENESS_KEY", Column::SHORT, &m_siUNIQUENESS_KEY);
         hQuery.bind("EMS_CASE", "PAN", Column::STRING, &m_strPAN, 0, 0, 2);
         hQuery.bind("EMS_CASE", "RETRIEVAL_REF_NO", Column::STRING, &m_strRETRIEVAL_REF_NO);
         hQuery.bind("EMS_CASE", "SYS_TRACE_AUDIT_NO", Column::STRING, &m_strSYS_TRACE_AUDIT_NO);
         hQuery.bind("EMS_CASE", "AMT_TRAN", Column::DOUBLE, &m_dAMT_TRAN);
         hQuery.setBasicPredicate("EMS_CASE", "CASE_NO", "=", m_pXMLItem->get("caseNumber").c_str());
         bool b = pSelectStatement->execute(hQuery);
         if (b == false
            || pSelectStatement->getRows() == 0 || m_strPAN.empty())
         {
            m_pXMLDocument->revert();
            SOAPSegment::instance()->setRtnCde(b ? '2' : '5');
            m_pXMLDocument->add("details");
            return reply();
         }
      }
      m_hQuery.reset();
      m_iQuery = 2;
      m_hRelatedCaseSegment.bind(m_hQuery);
      //RelatedCaseSegment::getCUR_TRAN();
      //RelatedCaseSegment::instance()->bind(m_hQuery);
      m_hQuery.bind("EMS_CASE", "NET_RULES", Column::STRING, &m_strNET_RULES);
      if (!m_pXMLItem->get("primaryKey").empty())
         m_hQuery.setBasicPredicate("EMS_CASE", "TSTAMP_TRANS", "=", m_pXMLItem->get("primaryKey").c_str());
      else
      {
         if (m_strTSTAMP_TRANS.length())
         {
            m_hQuery.setBasicPredicate("EMS_CASE", "TSTAMP_TRANS", "=", m_strTSTAMP_TRANS.c_str());
            m_hQuery.setBasicPredicate("EMS_CASE", "UNIQUENESS_KEY", "=", m_siUNIQUENESS_KEY);
            m_hQuery.setBasicPredicate("EMS_CASE", "CASE_NO", "<>", m_pXMLItem->get("caseNumber").c_str());
         }
         else
         {
            m_hQuery.setBasicPredicate("EMS_CASE", "PAN", "=", m_strPAN.c_str());
            m_hQuery.setBasicPredicate("EMS_CASE", "RETRIEVAL_REF_NO", "=", m_strRETRIEVAL_REF_NO.c_str());
            m_hQuery.setBasicPredicate("EMS_CASE", "SYS_TRACE_AUDIT_NO", "=", m_strSYS_TRACE_AUDIT_NO.c_str());
            m_hQuery.setBasicPredicate("EMS_CASE", "AMT_TRAN", "=", m_dAMT_TRAN);
            m_hQuery.setBasicPredicate("EMS_CASE", "CASE_NO", "<>", m_pXMLItem->get("caseNumber").c_str());
         }
      }
      m_hQuery.attach(this);
      bool b = pSelectStatement->execute(m_hQuery);
      if (b == false
         || pSelectStatement->getRows() == 0)
      {
         m_pXMLDocument->revert();
         SOAPSegment::instance()->setRtnCde('2');
      }
   }
   else
      SOAPSegment::instance()->setRtnCde('3');
   m_pXMLDocument->add("details");
   return reply();
  //## end restcommand::RelatedCaseCommand::execute%67BEF260020A.body
}

void RelatedCaseCommand::update (Subject* pSubject)
{
  //## begin restcommand::RelatedCaseCommand::update%67BEF287027D.body preserve=yes
   if (m_iQuery == 2)
   {
      ++m_iRows;
      ++m_iTotalRows;
      string strState;
      strState.reserve(30);
      strState = "STATE:";
      strState += m_hRelatedCaseSegment.getREQUEST_TYPE();
      strState += m_hRelatedCaseSegment.getSTATUS();
      CaseSegment::instance()->setNET_RULES(m_strNET_RULES);
      RulesEngine::instance()->getDescription(strState, m_strStateDescription);
      m_hRelatedCaseSegment.setStateDescription(m_strStateDescription);
      m_hGenericSegment.set("CaseNumber", m_hRelatedCaseSegment.getCASE_NO());
      m_hGenericSegment.set("CaseTypeIndicator", m_hRelatedCaseSegment.getCASE_TYPE_IND());
      m_hGenericSegment.set("StateDescription", m_hRelatedCaseSegment.getStateDescription());
      m_hGenericSegment.set("TransactionAmount", m_hRelatedCaseSegment.getAMT_TRAN());
      m_hGenericSegment.set("AdjustmentAmount", m_hRelatedCaseSegment.getAMT_ADJUSTMENT());
      m_hRelatedCaseSegment.setStateDescription(m_strStateDescription);
      //m_hGenericSegment.set("CaseNumber", "20250604000001");
      //m_hGenericSegment.set("CaseTypeIndicator", "X");
      //m_hGenericSegment.set("StateDescription", "CRS");
      //m_hGenericSegment.set("TransactionAmount", "2500");
      //m_hGenericSegment.set("AdjustmentAmount", "1000");

      m_pXMLDocument->add("row");
      UseCase::addItem();
      return;
   }
   command::RESTCommand::update(pSubject);
  //## end restcommand::RelatedCaseCommand::update%67BEF287027D.body
}

// Additional Declarations
  //## begin restcommand::RelatedCaseCommand%67BEF0690257.declarations preserve=yes
  //## end restcommand::RelatedCaseCommand%67BEF0690257.declarations

} // namespace restcommand

//## begin module%67BEEF610034.epilog preserve=yes
//## end module%67BEEF610034.epilog
