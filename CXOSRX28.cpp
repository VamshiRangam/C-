...
bool ISTTC57TransactionFile::distribute (string& strDATA_BUFFER, IF::FlatFile& hFlatFile)
{
  //## begin report::ISTTC57TransactionFile::distribute%6809C3C000C6.body preserve=yes
   ...
   if (Extract::instance()->getCustomCode() == "PAAS"
      && (m_hGenericSegment[0].get("MERCH_TYPE") == "4829" || m_hGenericSegment[0].get("MERCH_TYPE") == "6540"))
   {
      if (!memcmp(m_szADL_DATA_PRIV_ACQ, "MC", 2))
      {
         ...
         if (!memcmp(pADL_DATA_PRIV_ACQ->sPaymentTypeIndicator, "F07", 3))
         {
            write_TCR4_PaymentTransaction0_MC(hFlatFile);
            write_TCR4_PaymentTransaction1_MC(hFlatFile);
         }
      }
      else if (!memcmp(m_szADL_DATA_PRIV_ACQ, "VE", 2))
      {
         ...
         if (!memcmp(pADL_DATA_PRIV_ACQ->sBusinessApplicationID, "FT", 2))
            write_TCR3_BusAppDetails_Visa(hFlatFile);
      }
   }
   if (m_hGenericSegment[0].get("PROC_ID_ISS_B") == "MCI"
      || m_hGenericSegment[0].get("PROC_ID_ISS_B") == "MCINZ"
   ...
   if(Extract::instance()->getCustomCode() == "PAAS")
      write_TCR97_CommonMerchantData(hFlatFile);
...
bool ISTTC57TransactionFile::write_TCR0_TxnDetail (IF::FlatFile& hFlatFile)
{
  //## begin report::ISTTC57TransactionFile::write_TCR0_TxnDetail%6809C3C000D4.body preserve=yes
   ...
   else
   {
      ...
      string strTRAN_UNIQUE_DATA(m_hGenericSegment[0].get("TRAN_UNIQUE_DATA"));
      strTRAN_UNIQUE_DATA.resize(50, ' ');
      pTC57_TCR0_TxnDetail->cAVSResponseCode = strTRAN_UNIQUE_DATA[29];
   ...
   if (m_hGenericSegment[0].get("ACQ_PLAT_PROD_ID") == "A")
   {
      ...
      if (Extract::instance()->getCustomCode() == "PAAS" && (m_hGenericSegment[0].get("REF_DATA_ISS_FMT") == "4"))
      {
         visabaseii::segREF_DATA_ISS_EVES* pREF_DATA_ISS_EVES = (visabaseii::segREF_DATA_ISS_EVES*)m_szREF_DATA_ISS;
         pTC57_TCR0_TxnDetail->cAdditionalTknRespInfo = pREF_DATA_ISS_EVES->cAdditionalTokenResponseInfo;
      }
      if (strPROC_ID_ISS_B == "VISA" || m_hGenericSegment[0].get("NET_ID_ISS") == "VNT")
      {
...
bool ISTTC57TransactionFile::write_TCR1_TxnDetail_Common (IF::FlatFile& hFlatFile)
{
  //## begin report::ISTTC57TransactionFile::write_TCR1_TxnDetail_Common%6809C3C000D6.body preserve=yes
   ...
   if (Extract::instance()->getCustomCode() == "PAAS")
   {
      ...
      size_t pos = strADL_DATA_NATIONAL.find("\\M4");
      memset(pTC57_TCR1_TxnDetail_Common->sCustomerServicePhoneNum, ' ', sizeof(pTC57_TCR1_TxnDetail_Common->sCustomerServicePhoneNum));
      if (pos != string::npos)
      {
         int iLen = atoi(strADL_DATA_NATIONAL.substr(pos + 3, 2).data());
         if (iLen && strADL_DATA_NATIONAL.length() >= (pos + 5 + iLen))
            memcpy(pTC57_TCR1_TxnDetail_Common->sCustomerServicePhoneNum, strADL_DATA_NATIONAL.data() + pos + 5, min(size_t(13), size_t(iLen)));
      }
   }
...
bool ISTTC57TransactionFile::write_TCR5_TxnDetail_MC (IF::FlatFile& hFlatFile)
{
  //## begin report::ISTTC57TransactionFile::write_TCR5_TxnDetail_MC%6809C3C000DE.body preserve=yes
   ...
   char szECOM_DATA[96];
   memset(szECOM_DATA, ' ', sizeof(szECOM_DATA) - 1);
   szECOM_DATA[sizeof(szECOM_DATA) - 1] = '\0';
   memcpy(szECOM_DATA, m_hGenericSegment[0].get("ECOM_DATA").data(), m_hGenericSegment[0].get("ECOM_DATA").length());
   if (!memcmp(szECOM_DATA, "05", 2))
   {
      mastercardcis::segECOM_DATA* pECOM_DATA;
      pECOM_DATA = (struct mastercardcis::segECOM_DATA*)szECOM_DATA;
      ...
      pTC57_TCR5_TxnDetail_MC->cDigcomident = pECOM_DATA->cDigcomident;
      pTC57_TCR5_TxnDetail_MC->cDigcomprotype = pECOM_DATA->cDigcomprotype;
      memcpy(pTC57_TCR5_TxnDetail_MC->sDigcometype, pECOM_DATA->sDigcometype, sizeof(pTC57_TCR5_TxnDetail_MC->sDigcometype));
      memcpy(pTC57_TCR5_TxnDetail_MC->sDigcomchkmtd, pECOM_DATA->sDigcomchkmtd, sizeof(pTC57_TCR5_TxnDetail_MC->sDigcomchkmtd));
   }
   string strToValue;
   if (m_hGenericSegment[0].get("ACQ_PLAT_PROD_ID") != "A")
   {
   ...
   if (m_hGenericSegment[0].get("ACQ_PLAT_PROD_ID") == "A")
   {
      if (!memcmp(m_szADL_DATA_PRIV_ACQ, "MC", 2))
      {
         ...
         if (Extract::instance()->getCustomCode() == "PAAS")
         {
            ...
            memcpy(pTC57_TCR5_TxnDetail_MC->sTransactionLinkID, pADL_DATA_PRIV_ISS->sTransactionLinkID, sizeof(pTC57_TCR5_TxnDetail_MC->sTransactionLinkID));
         }
         else
         {
            memcpy(pTC57_TCR5_TxnDetail_MC->sTransactionLinkID, pADL_DATA_PRIV_ACQ->sTransactionLinkID, sizeof(pTC57_TCR5_TxnDetail_MC->sTransactionLinkID));
         }
         if (pADL_DATA_PRIV_ACQ->sPymtIntChannelDevType[0] != ' ' && pADL_DATA_PRIV_ACQ->sPymtIntChannelDevType[0] != '\0')
            memcpy(pTC57_TCR5_TxnDetail_MC->sDeviceTypeIndicator, pADL_DATA_PRIV_ACQ->sPymtIntChannelDevType, sizeof(pTC57_TCR5_TxnDetail_MC->sDeviceTypeIndicator));
         else if (!m_hGenericSegment[0].get("FORM_FACTOR_IND").empty()
            && m_hGenericSegment[0].get("FORM_FACTOR_IND").length() > 12)
         {
            string strDeviceTypeIndHex(m_hGenericSegment[0].get("FORM_FACTOR_IND").data() + 8, 4); // hex 3230 -> ascii 20
            unsigned char szTemp[3];
            CodeTable::byteToNibble(strDeviceTypeIndHex, szTemp);
#ifdef MVS
            CodeTable::translate((char*)szTemp, 2, CodeTable::CX_ASCII_TO_EBCDIC);
#endif
            memcpy(pTC57_TCR5_TxnDetail_MC->sDeviceTypeIndicator, szTemp, sizeof(pTC57_TCR5_TxnDetail_MC->sDeviceTypeIndicator));
         }
         pTC57_TCR5_TxnDetail_MC->cMasOneCredExcInd = pADL_DATA_PRIV_ACQ->cMasOneCredExcInd;
         memcpy(pTC57_TCR5_TxnDetail_MC->sProgramRegistrationId, pADL_DATA_PRIV_ACQ->sPaymentTypeIndicator, sizeof(pTC57_TCR5_TxnDetail_MC->sProgramRegistrationId));
      }
      else if (!memcmp(m_szADL_DATA_PRIV_ACQ, "MD", 2))
      {
         mastercardmds::segADL_DATA_PRIV_ACQ* pADL_DATA_PRIV_ACQ;
   ...
   if (Token::parse(strALMData.length(), strALMData, 1, hTokenMap, 2))
   {
      if (hTokenMap.count("01"))
         memcpy(pTC57_TCR5_TxnDetail_MC->sALMServiceCode, hTokenMap["01"].data(), min(sizeof(pTC57_TCR5_TxnDetail_MC->sALMServiceCode), hTokenMap["01"].length()));
      if (hTokenMap.count("02"))
         memcpy(pTC57_TCR5_TxnDetail_MC->sALMProductCode, hTokenMap["02"].data(), min(sizeof(pTC57_TCR5_TxnDetail_MC->sALMProductCode), hTokenMap["02"].length()));
      if (hTokenMap.count("03"))
         memcpy(pTC57_TCR5_TxnDetail_MC->sALMProductClass, hTokenMap["03"].data(), min(sizeof(pTC57_TCR5_TxnDetail_MC->sALMProductClass), hTokenMap["03"].length()));
      if (hTokenMap.count("04"))
         memcpy(pTC57_TCR5_TxnDetail_MC->sALMRateType, hTokenMap["04"].data(), min(sizeof(pTC57_TCR5_TxnDetail_MC->sALMRateType), hTokenMap["04"].length()));
      if (hTokenMap.count("05"))
         memcpy(pTC57_TCR5_TxnDetail_MC->sMasOneCredAcceBrand, hTokenMap["05"].data(), min(sizeof(pTC57_TCR5_TxnDetail_MC->sMasOneCredAcceBrand), hTokenMap["05"].length()));
      if (hTokenMap.count("06"))
         pTC57_TCR5_TxnDetail_MC->cMasOneCredIntchgInd = hTokenMap["06"][0];
      if (hTokenMap.count("07"))
         memcpy(pTC57_TCR5_TxnDetail_MC->sMasOneCredIssAcctRnge, hTokenMap["07"].data(), min(sizeof(pTC57_TCR5_TxnDetail_MC->sMasOneCredIssAcctRnge), hTokenMap["07"].length()));
   }
...
bool ISTTC57TransactionFile::write_TCR3_TxnDetail_PaymentServiceData (IF::FlatFile& hFlatFile)
{
  //## begin report::ISTTC57TransactionFile::write_TCR3_TxnDetail_PaymentServiceData%6809C3C000E4.body preserve=yes
   ...
   else
   {
      ...
      strCryptogramAmount = m_hGenericSegment[0].get("CRYPTOGRAM_AMOUNT");
      ...
      if (m_hGenericSegment[0].get("ACQ_PLAT_PROD_ID") == "A")
      {
         ...
         if (m_hGenericSegment[0].get("REF_DATA_ISS_FMT") == "4")
         {
            ...
            {
               visabaseii::segREF_DATA_ISS_EVES_AUS* pREF_DATA_ISS_EVES_AUS = (visabaseii::segREF_DATA_ISS_EVES_AUS*)m_szREF_DATA_ISS;
               pTC57_TCR3_TxnDetail_PSD->cSpendQualInd = pREF_DATA_ISS_EVES_AUS->cSpendQualifiedIndicator;
            }
         }
...
bool ISTTC57TransactionFile::write_TCR4_PaymentTransaction1_MC (IF::FlatFile& hFlatFile)
{
  //## begin report::ISTTC57TransactionFile::write_TCR4_PaymentTransaction1_MC%68B56BC100EB.body preserve=yes
   m_iNoOfTcrs++;
   m_iTotNoOfTcrs++;
   m_siIntrMerchBatchKey++;
   struct hTC57_TCR4_PaymentTransaction1_MC* pTC57_TCR4_PaymentTransaction1_MC = (struct hTC57_TCR4_PaymentTransaction1_MC*)getDATA_BUFFER();
   memset(pTC57_TCR4_PaymentTransaction1_MC, ' ', sizeof(hTC57_TCR4_PaymentTransaction1_MC));
   memcpy(pTC57_TCR4_PaymentTransaction1_MC->sTransCode, "57", sizeof(pTC57_TCR4_PaymentTransaction1_MC->sTransCode));
   pTC57_TCR4_PaymentTransaction1_MC->cTransCodeQualifier = '0';
   pTC57_TCR4_PaymentTransaction1_MC->cTransComponentSeqNumber = '4';
   memcpy(pTC57_TCR4_PaymentTransaction1_MC->sBusinessFormatCode, "P1", sizeof(pTC57_TCR4_PaymentTransaction1_MC->sBusinessFormatCode));
   pTC57_TCR4_PaymentTransaction1_MC->cRecordType = '2';
   memcpy(pTC57_TCR4_PaymentTransaction1_MC->sAdditionalTraceRefNum, m_hGenericSegment[0].get("SEG24_527^R1").data(), min(size_t(19), m_hGenericSegment[0].get("SEG24_527^R1").length()));
   return (write(sizeof(hTC57_TCR4_PaymentTransaction1_MC), hFlatFile));
  //## end report::ISTTC57TransactionFile::write_TCR4_PaymentTransaction1_MC%68B56BC100EB.body
}

bool ISTTC57TransactionFile::write_TCR4_PaymentTransaction0_MC (IF::FlatFile& hFlatFile)
{
  //## begin report::ISTTC57TransactionFile::write_TCR4_PaymentTransaction0_MC%68E35DFA00BB.body preserve=yes
   m_iNoOfTcrs++;
   m_iTotNoOfTcrs++;
   m_siIntrMerchBatchKey++;
   struct hTC57_TCR4_PaymentTransaction0_MC* pTC57_TCR4_PaymentTransaction0_MC = (struct hTC57_TCR4_PaymentTransaction0_MC*)getDATA_BUFFER();
   memset(pTC57_TCR4_PaymentTransaction0_MC, ' ', sizeof(hTC57_TCR4_PaymentTransaction0_MC));
   memcpy(pTC57_TCR4_PaymentTransaction0_MC->sTransCode, "57", sizeof(pTC57_TCR4_PaymentTransaction0_MC->sTransCode));
   pTC57_TCR4_PaymentTransaction0_MC->cTransCodeQualifier = '0';
   pTC57_TCR4_PaymentTransaction0_MC->cTransComponentSeqNumber = '4';
   memcpy(pTC57_TCR4_PaymentTransaction0_MC->sBusinessFormatCode, "P0", sizeof(pTC57_TCR4_PaymentTransaction0_MC->sBusinessFormatCode));
   memcpy(pTC57_TCR4_PaymentTransaction0_MC->sFundRequestDate, m_hGenericSegment[0].get("TSTAMP_LOCAL").data() + 2, 6);
   return (write(sizeof(hTC57_TCR4_PaymentTransaction0_MC), hFlatFile));
  //## end report::ISTTC57TransactionFile::write_TCR4_PaymentTransaction0_MC%68E35DFA00BB.body
}

bool ISTTC57TransactionFile::write_TCR3_BusAppDetails_Visa (IF::FlatFile& hFlatFile)
{
  //## begin report::ISTTC57TransactionFile::write_TCR3_BusAppDetails_Visa%68B56BF101DB.body preserve=yes
   m_iNoOfTcrs++;
   m_iTotNoOfTcrs++;
   m_siIntrMerchBatchKey++;
   struct hTC57_TCR3_BusAppDetails_Visa* pTC57_TCR3_BusAppDetails_Visa = (struct hTC57_TCR3_BusAppDetails_Visa*)getDATA_BUFFER();
   memset(pTC57_TCR3_BusAppDetails_Visa, ' ', sizeof(hTC57_TCR3_BusAppDetails_Visa));
   memcpy(pTC57_TCR3_BusAppDetails_Visa->sTransCode, "57", sizeof(pTC57_TCR3_BusAppDetails_Visa->sTransCode));
   pTC57_TCR3_BusAppDetails_Visa->cTransCodeQualifier = '1'; //Account Funding
   pTC57_TCR3_BusAppDetails_Visa->cTransComponentSeqNumber = '3';
   memcpy(pTC57_TCR3_BusAppDetails_Visa->sServiceProcessingType, "00", sizeof(pTC57_TCR3_BusAppDetails_Visa->sServiceProcessingType));
   memcpy(pTC57_TCR3_BusAppDetails_Visa->sBusinessFormatCode, "CR", sizeof(pTC57_TCR3_BusAppDetails_Visa->sBusinessFormatCode));
   pTC57_TCR3_BusAppDetails_Visa->cSourceOfFunds = '3';
   visabaseii::segADL_DATA_PRIV_ACQ* pADL_DATA_PRIV_ACQ; //VE%
   pADL_DATA_PRIV_ACQ = (struct visabaseii::segADL_DATA_PRIV_ACQ*)m_szADL_DATA_PRIV_ACQ;
   memcpy(pTC57_TCR3_BusAppDetails_Visa->sBusinessAppId, pADL_DATA_PRIV_ACQ->sBusinessApplicationID, sizeof(pTC57_TCR3_BusAppDetails_Visa->sBusinessAppId));
   return (write(sizeof(hTC57_TCR3_BusAppDetails_Visa), hFlatFile));
  //## end report::ISTTC57TransactionFile::write_TCR3_BusAppDetails_Visa%68B56BF101DB.body
}

bool ISTTC57TransactionFile::write_TCR97_CommonMerchantData (IF::FlatFile& hFlatFile)
{
  //## begin report::ISTTC57TransactionFile::write_TCR97_CommonMerchantData%68B56C1D00AB.body preserve=yes
   m_iNoOfTcrs++;
   m_iTotNoOfTcrs++;
   m_siIntrMerchBatchKey++;
   struct hTC57_TCR97_CommonMerchantData* pTC57_TCR97_CommonMerchantData = (struct hTC57_TCR97_CommonMerchantData*)getDATA_BUFFER();
   memset(pTC57_TCR97_CommonMerchantData, ' ', sizeof(hTC57_TCR97_CommonMerchantData));
   memcpy(pTC57_TCR97_CommonMerchantData->sTransCode, "57", sizeof(pTC57_TCR97_CommonMerchantData->sTransCode));
   memcpy(pTC57_TCR97_CommonMerchantData->sTransCodeQualifier, "97", sizeof(pTC57_TCR97_CommonMerchantData->sTransCodeQualifier));
   memcpy(pTC57_TCR97_CommonMerchantData->sFormatCode, "CM", sizeof(pTC57_TCR97_CommonMerchantData->sFormatCode));
   memcpy(pTC57_TCR97_CommonMerchantData->sCardAcceptorId, m_hGenericSegment[0].get("RPT_LVL_ID_B").data(), min(size_t(16), m_hGenericSegment[0].get("RPT_LVL_ID_B").length()));
   memcpy(pTC57_TCR97_CommonMerchantData->sMerchantStateCode, m_hGenericSegment[0].get("CARD_ACPT_REGION").data(), min(size_t(3), m_hGenericSegment[0].get("CARD_ACPT_REGION").length()));
   memcpy(pTC57_TCR97_CommonMerchantData->sMerchantCountryCode, m_hGenericSegment[0].get("CARD_ACPT_COUNTRY").data(), min(size_t(3), m_hGenericSegment[0].get("CARD_ACPT_COUNTRY").length()));
   memcpy(pTC57_TCR97_CommonMerchantData->sMerchantZipCode, m_hGenericSegment[0].get("CARD_ACPT_PST_CODE").data(), min(size_t(16), m_hGenericSegment[0].get("CARD_ACPT_PST_CODE").length()));
   memcpy(pTC57_TCR97_CommonMerchantData->sStoreTerminalNum, m_hGenericSegment[0].get("CARD_ACPT_TERM_ID").data(), min(size_t(8), m_hGenericSegment[0].get("CARD_ACPT_TERM_ID").length()));
   memcpy(pTC57_TCR97_CommonMerchantData->sMerchantCategoryCode, m_hGenericSegment[0].get("MERCH_TYPE").data(), min(size_t(4), m_hGenericSegment[0].get("MERCH_TYPE").length()));
   if (!m_hGenericSegment[0].get("CARD_ACPT_NAME_LOC").empty())
   {
       memcpy(pTC57_TCR97_CommonMerchantData->sMerchantName, m_hGenericSegment[0].get("CARD_ACPT_NAME_LOC").data(), min(size_t(25), m_hGenericSegment[0].get("CARD_ACPT_NAME_LOC").length()));
       if (m_hGenericSegment[0].get("CARD_ACPT_NAME_LOC").length() > 26)
       {
           string strMerchantStreet(m_hGenericSegment[0].get("CARD_ACPT_NAME_LOC").data() + 26, min(size_t(29), m_hGenericSegment[0].get("CARD_ACPT_NAME_LOC").length() - 26));
           trim(strMerchantStreet);
           memcpy(pTC57_TCR97_CommonMerchantData->sMerchantStreetNum, strMerchantStreet.data(), strMerchantStreet.length());
       }
       if (m_hGenericSegment[0].get("CARD_ACPT_NAME_LOC").length() > 55)
       {
           string strMerchantCity(m_hGenericSegment[0].get("CARD_ACPT_NAME_LOC").data() + 55, min(size_t(13), m_hGenericSegment[0].get("CARD_ACPT_NAME_LOC").length() - 55));
           trim(strMerchantCity);
           memcpy(pTC57_TCR97_CommonMerchantData->sMerchantCity, strMerchantCity.data(), strMerchantCity.length());
       }
   }
   string strADL_DATA_NATIONAL(m_hGenericSegment[0].get("ADL_DATA_NATIONAL"));
   size_t pos = strADL_DATA_NATIONAL.find("\\M3");
   if (pos != string::npos)
   {
       int iLen = atoi(strADL_DATA_NATIONAL.substr(pos + 3, 2).data());
       if (iLen && strADL_DATA_NATIONAL.length() >= (pos + 5 + iLen))
           memcpy(pTC57_TCR97_CommonMerchantData->sMerchantURL, strADL_DATA_NATIONAL.data() + pos + 5, iLen);
   }
   return (write(sizeof(hTC57_TCR97_CommonMerchantData), hFlatFile));
  //## end report::ISTTC57TransactionFile::write_TCR97_CommonMerchantData%68B56C1D00AB.body
}
