...
void TC57TransactionFile::setCATLevelInd (string& strValue)
{
  //## begin report::TC57TransactionFile::setCATLevelInd%67F3EF2803CD.body preserve=yes
   ...
   if (Extract::instance()->getCustomCode() == "CBA" && m_pGenericSegment->get("REF_DATA_ISS_FMT") == "6")
   {
      mastercardcis::segREF_DATA_ISS* pREF_DATA_ISS;
      pREF_DATA_ISS = (struct mastercardcis::segREF_DATA_ISS*)m_pGenericSegment->get("REF_DATA_ISS").data();
      strValue = pREF_DATA_ISS->sPOS_DATA_CODE[9];
   }
  ...
  //## begin report::TC57TransactionFile::write_TCR0_TxnDetail%626103F00319.body preserve=yes
   ...
   if (m_hGenericSegment[0].get("ACQ_PLAT_PROD_ID") == "A")
   {
      ...
      if (Extract::instance()->getCustomCode() == "PAAS" && (m_hGenericSegment[0].get("REF_DATA_ISS_FMT") == "4"))
      {
         ...
         m_hGenericSegment[0].set("ZTOKENRESPINFO", string(&pREF_DATA_ISS_EVES->cAdditionalTokenResponseInfo, 1));
      }
      if (strPROC_ID_ISS_B == "VISA" || m_hGenericSegment[0].get("NET_ID_ISS") == "VNT")
      {
...
bool TC57TransactionFile::write_TCR1_TxnDetail_Common (IF::FlatFile& hFlatFile)
{
  //## begin report::TC57TransactionFile::write_TCR1_TxnDetail_Common%626103F501BF.body preserve=yes
   ...
   if (Extract::instance()->getCustomCode() == "PAAS")
   {
      size_t pos = m_hGenericSegment[0].get("ADL_DATA_NATIONAL").find("\\M4");
      if (pos != string::npos)
      {
         ...
         if (iLen && m_hGenericSegment[0].get("ADL_DATA_NATIONAL").length() >= (pos + 5 + iLen))
            m_hGenericSegment[0].set("ZMERCHPHONE", m_hGenericSegment[0].get("ADL_DATA_NATIONAL").substr(pos + 5, iLen));
      }
   }
...
bool TC57TransactionFile::write_TCR3_TxnDetail_PaymentServiceData (IF::FlatFile& hFlatFile)
{
  //## begin report::TC57TransactionFile::write_TCR3_TxnDetail_PaymentServiceData%6268B2160286.body preserve=yes
   ...
   {
      string strValue;
      visasms::segREF_DATA_ISS* pREF_DATA_ISS = (struct visasms::segREF_DATA_ISS*)m_szREF_DATA_ISS;
      ...
      if (m_hGenericSegment[0].get("ACQ_PLAT_PROD_ID") == "A")
      {
         ...
         if (m_hGenericSegment[0].get("REF_DATA_ISS_FMT") == "4")
         {
            ...
            if (Extract::instance()->getCustomCode() == "CBA")
            {
               ...
               strValue = pREF_DATA_ISS_EVES_AUS->cSpendQualifiedIndicator;
               m_hGenericSegment[0].set("ZSSQUALIND", strValue);
            }
         }
...
bool TC57TransactionFile::write_TCR5_TxnDetail_MC (IF::FlatFile& hFlatFile)
{
  //## begin report::TC57TransactionFile::write_TCR5_TxnDetail_MC%6261045500ED.body preserve=yes
   ...
   if (!memcmp(szECOM_DATA, "05", 2))
   {
      ...
      char szCIMICAT[4];
      memset(szCIMICAT, ' ', 4);
      memcpy(szCIMICAT, pECOM_DATA->sCitMitCategory, sizeof(pECOM_DATA->sCitMitCategory));
      memcpy(szCIMICAT + 2 , pECOM_DATA->sCitMitSubcategory, sizeof(pECOM_DATA->sCitMitSubcategory));
      m_hGenericSegment[0].set("ZCIMICAT", szCIMICAT);
      char szDIGCOM[57];
      memset(szDIGCOM, ' ', 57);
      szDIGCOM[0] = pECOM_DATA->cDigcomident;
      szDIGCOM[1] = pECOM_DATA->cDigcomprotype;
      memcpy(szDIGCOM + 2 , pECOM_DATA->sDigcometype, sizeof(pECOM_DATA->sDigcometype));
      memcpy(szDIGCOM + 17, pECOM_DATA->sDigcomchkmtd, sizeof(pECOM_DATA->sDigcomchkmtd));
      m_hGenericSegment[0].set("ZDIGCOM", szDIGCOM);
   }
   if (Extract::instance()->getCustomCode() == "COLES")
   {
      ...
      mastercardmds::segADL_DATA_PRIV_ACQ* pADL_DATA_PRIV_ACQ;
      pADL_DATA_PRIV_ACQ = (struct mastercardmds::segADL_DATA_PRIV_ACQ*)m_szADL_DATA_PRIV_ACQ;
      strValue.assign(pADL_DATA_PRIV_ACQ->sTransactionLinkID, sizeof(pADL_DATA_PRIV_ACQ->sTransactionLinkID));
      m_hGenericSegment[0].set("ZTRAN_LINK_ID", strValue);
   }
   else
   {
   ...
   if (m_hGenericSegment[0].get("ACQ_PLAT_PROD_ID") == "A")
   {
      if (!memcmp(m_szADL_DATA_PRIV_ACQ, "MC", 2))
      {
         ...
         if (Extract::instance()->getCustomCode() == "PAAS")
         {
            ...
            strValue1.assign(pADL_DATA_PRIV_ISS->sTransactionLinkID, sizeof(pADL_DATA_PRIV_ISS->sTransactionLinkID));
         ...
         {
            strValue1.assign(pADL_DATA_PRIV_ACQ->sPymtIntChannelDevType, sizeof(pADL_DATA_PRIV_ACQ->sPymtIntChannelDevType));
            m_hGenericSegment[0].set("ZDEVICE_TYPE", strValue1);
         }
...
#endif
            strValue1.assign((const char*)szTemp,2);
            m_hGenericSegment[0].set("ZDEVICE_TYPE", strValue1);
         }
         strValue1 = pADL_DATA_PRIV_ACQ->cMasOneCredExcInd;
         m_hGenericSegment[0].set("ZONE_CRED_EXC_IND", strValue1);
         strValue1.assign(pADL_DATA_PRIV_ACQ->sPaymentTypeIndicator, sizeof(pADL_DATA_PRIV_ACQ->sPaymentTypeIndicator));
   ...
   if (Token::parse(strALMData.length(), strALMData, 1, hTokenMap, 2))
   {
      ...
      {
         strValue1.assign(hTokenMap["01"].data(), min<size_t>(5, hTokenMap["01"].length()));
         m_hGenericSegment[0].set("ZAL01", strValue1);
      }
      if (hTokenMap.count("02"))
      {
         strValue1.assign(hTokenMap["02"].data(), min<size_t>(3, hTokenMap["02"].length()));
         m_hGenericSegment[0].set("ZAL02", strValue1);
      }
      if (hTokenMap.count("03"))
      {
         strValue1.assign(hTokenMap["03"].data(), min<size_t>(3, hTokenMap["03"].length()));
         m_hGenericSegment[0].set("ZAL03", strValue1);
      }
      if (hTokenMap.count("04"))
      {
         strValue1.assign(hTokenMap["04"].data(), min<size_t>(3, hTokenMap["04"].length()));
         m_hGenericSegment[0].set("ZAL04", strValue1);
      }
      if (hTokenMap.count("05"))
      {
         strValue1.assign(hTokenMap["05"].data(), min<size_t>(3, hTokenMap["05"].length()));
         m_hGenericSegment[0].set("ZAL05", strValue1);
      }
      if (hTokenMap.count("06"))
      {
         strValue1.assign(hTokenMap["06"].data(), min<size_t>(1, hTokenMap["06"].length()));
         m_hGenericSegment[0].set("ZAL06", strValue1);
      }
      if (hTokenMap.count("07"))
      {
         strValue1.assign(hTokenMap["07"].data(), min<size_t>(19, hTokenMap["07"].length()));
         m_hGenericSegment[0].set("ZAL07", strValue1);
      }
   }
...
bool TC57TransactionFile::write_TCR4_PaymentTransaction1_MC (IF::FlatFile& hFlatFile)
{
  //## begin report::TC57TransactionFile::write_TCR4_PaymentTransaction1_MC%68B64F030275.body preserve=yes
   m_hActivitySegment[2].reset();
   m_hActivitySegment[2].set("BKEY", 1);
   m_hActivitySegment[2].set("TCR", 1);
   m_hActivitySegment[2].set("TTCR", 1);
   m_hActivitySegment[0] += m_hActivitySegment[2];
   return m_pReport->report("P1");
  //## end report::TC57TransactionFile::write_TCR4_PaymentTransaction1_MC%68B64F030275.body
}

bool TC57TransactionFile::write_TCR4_PaymentTransaction0_MC (IF::FlatFile& hFlatFile)
{
  //## begin report::TC57TransactionFile::write_TCR4_PaymentTransaction0_MC%68E35F88000A.body preserve=yes
   m_hActivitySegment[2].reset();
   m_hActivitySegment[2].set("BKEY", 1);
   m_hActivitySegment[2].set("TCR", 1);
   m_hActivitySegment[2].set("TTCR", 1);
   m_hActivitySegment[0] += m_hActivitySegment[2];
   return m_pReport->report("P0");
  //## end report::TC57TransactionFile::write_TCR4_PaymentTransaction0_MC%68E35F88000A.body
}

bool TC57TransactionFile::write_TCR3_BusAppDetails_Visa (IF::FlatFile& hFlatFile)
{
  //## begin report::TC57TransactionFile::write_TCR3_BusAppDetails_Visa%68B64F1C032E.body preserve=yes
   m_hActivitySegment[2].reset();
   m_hActivitySegment[2].set("BKEY", 1);
   m_hActivitySegment[2].set("TCR", 1);
   m_hActivitySegment[2].set("TTCR", 1);
   ...
   if (!memcmp(m_szADL_DATA_PRIV_ACQ, "VE", 2))
   {
      ...
      visabaseii::segADL_DATA_PRIV_ACQ* pADL_DATA_PRIV_ACQ;
      pADL_DATA_PRIV_ACQ = (struct visabaseii::segADL_DATA_PRIV_ACQ*)m_szADL_DATA_PRIV_ACQ;
      strValue.assign(pADL_DATA_PRIV_ACQ->sBusinessApplicationID, sizeof(pADL_DATA_PRIV_ACQ->sBusinessApplicationID));
      m_hGenericSegment[0].set("ZBUSAPID", strValue);
   }
   return m_pReport->report("BA");
  //## end report::TC57TransactionFile::write_TCR3_BusAppDetails_Visa%68B64F1C032E.body
}

bool TC57TransactionFile::write_TCR97_CommonMerchantData (IF::FlatFile& hFlatFile)
{
  //## begin report::TC57TransactionFile::write_TCR97_CommonMerchantData%68B64F29036C.body preserve=yes
   m_hActivitySegment[2].reset();
   m_hActivitySegment[2].set("BKEY", 1);
   m_hActivitySegment[2].set("TCR", 1);
   m_hActivitySegment[2].set("TTCR", 1);
   ...
   return m_pReport->report("CM");
  //## end report::TC57TransactionFile::write_TCR97_CommonMerchantData%68B64F29036C.body
}
